...
async function insertProductData(productId) {
  try {
    const response = await fetch(`http://34.64.249.228.228:8080/products/${productId}`);
    if (!response.ok) {
    ...
    addToCartButton.addEventListener("click", async () => {
      ...
      const result = await fetch(`http://34.64.249.228.228:8080/carts/user/${sessionUser}/items?product=${productId}`, {
        method: "POST",
...
async function fetchReviews(productId) {
  try {
    const response = await fetch(`http://34.64.249.228.228:8080/reviews/product/${productId}`);
    if (!response.ok) {
